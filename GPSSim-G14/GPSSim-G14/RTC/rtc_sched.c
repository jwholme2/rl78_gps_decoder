     ...
     if (GBL_task_list[i].enabled == 1) {
       if (GBL_task_list[i].delay) {
         if (--GBL_task_list[i].delay == 0){
           //Ready flag set...record time for latency start 
           GBL_task_list[i].lat_timer.start =  TCR00;
...
void Init_Task_Timers(void)
{
  ...
  for (i=0 ; i<MAX_TASKS ; i++) {
    ...
    GBL_task_list[i].exec_time_us.min = 0xffffffff;   
    GBL_task_list[i].exec_time_us.max = 0;        
    GBL_task_list[i].exec_time_us.num_samples = 0;   
    GBL_task_list[i].exec_time_us.total = 0;  
    GBL_task_list[i].latency_us.min = 0xffffffff;    
    GBL_task_list[i].latency_us.max = 0;     
    GBL_task_list[i].latency_us.num_samples = 0;   
    GBL_task_list[i].latency_us.total = 0;  
    GBL_task_list[i].lat_timer.start=0;    
    GBL_task_list[i].lat_timer.end=0;
				...
				if (GBL_task_list[i].enabled == 1) { /* enabled */
				  if (GBL_task_list[i].ready == 1) {  /* ready to run */
						...
						RTC_ACTIVE_OUTPUT = 0; // Indicate task is active
                                    ...
                                    //Read the time!
                                    start =  TCR00;
                                    GBL_task_list[i].lat_timer.end =  TCR00;
				    /* Run the task */
				    GBL_task_list[i].task();
                                    end = TCR00;
                                    
                                    //If we had a overflow...
                                    if ((start-end) < 0) {
                                      start +=65536;                   
                                    }
                                                                        
                                    //If we had a overflow...
                                    if ((GBL_task_list[i].lat_timer.start-GBL_task_list[i].lat_timer.end) < 0) {
                                      GBL_task_list[i].lat_timer.start +=65536;                   
                                    }
                                                                        
                                    //EXECUTION calculations
                                    //If it's higher than max, store it!
                                    if ((start-end) > GBL_task_list[i].exec_time_us.max) {
                                      GBL_task_list[i].exec_time_us.max = (unsigned short) (start-end);
                                      start=0;
                                      end=0;
                                    }
                                    if ((start-end) < GBL_task_list[i].exec_time_us.min) {
                                      GBL_task_list[i].exec_time_us.min = (unsigned short) (start-end);
                                    ...
                                    //If it's higher than max, store it!
                                    if ((GBL_task_list[i].lat_timer.start-GBL_task_list[i].lat_timer.end) > GBL_task_list[i].latency_us.max) {
                                      GBL_task_list[i].latency_us.max = (unsigned short) (GBL_task_list[i].lat_timer.start-GBL_task_list[i].lat_timer.end);
                                      GBL_task_list[i].lat_timer.start=0;                                     
                                      GBL_task_list[i].lat_timer.end=0;
                                    }
                                    if ((GBL_task_list[i].lat_timer.start-GBL_task_list[i].lat_timer.end) < GBL_task_list[i].latency_us.min) {
                                      GBL_task_list[i].latency_us.min = (unsigned short) (GBL_task_list[i].lat_timer.start-GBL_task_list[i].lat_timer.end);
                                      
                                      GBL_task_list[i].lat_timer.start=0;                                     
                                      GBL_task_list[i].lat_timer.end=0;
                                    }
                                    
                                    //Update the totals for our average calculations
                                    GBL_task_list[i].exec_time_us.total+= (unsigned short) (start-end);                                    
                                    GBL_task_list[i].latency_us.total+= (unsigned short) (GBL_task_list[i].lat_timer.start-GBL_task_list[i].lat_timer.end);
                                    GBL_task_list[i].exec_time_us.num_samples++;
                                    GBL_task_list[i].latency_us.num_samples++;
...
void Run_TaskN(int task_number)
{
  ...
  //Ready flag set...record time for latency start 
  GBL_task_list[task_number].lat_timer.start=TCR00;
